...
Spring Boot DevTools: spring-boot-devtools (optional)
H2 Database: h2 (for testing)
MySQL Driver: mysql-connector-j (for production/development)
Lombok: lombok
Spring Boot Test Starter: spring-boot-starter-test
Before coding the entities:

Initialize the git repository (if not done).
Commit the setup files (pom.xml, workshop files).
Run the Application: Verify that the application starts without errors and the database connection is successful.
Push the changes to GitHub.
Share the Link: Provide the repository URL to your instructor for initial review.
...
1. Address Entity (Unidirectional)
   ...
   Relationship: Note that this is a standalone entity in terms of mapping; it doesn't need to reference the Customer.
...
Address: Implement a Unidirectional One-to-One relationship. The foreign key column should be named address_id (mandatory).
UserProfile: Implement a Bidirectional One-to-One relationship (Optional). This is the owner side. The foreign key column should be named profile_id.
Optional: Cascading & Lifecycle:
Configure both relationships so that persistence operations (like save and delete) propagate from Customer to the associated entities.
Ensure that if an associated entity is removed from the Customer, it is also deleted from the database (orphan removal).
